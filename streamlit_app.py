...
if not df_raw.empty:
    ...
    col_visualizacion, col_espacio_der = st.columns([95, 5])
    
    with col_visualizacion:
        if not zona_analisis:
            st.title(f"📊 Dashboard Operativo — {mes_sel.upper()} ({opcion_region.upper()})")
            
            # 🌟 NUEVA EXIGENCIA: Subtítulo con formato de fecha de facturación y fecha última de actualización lado a lado
            ...
            if summary_metrics.empty or summary_metrics['Pedidos'].sum() == 0:
                ...
                # --- 6.2 DESGLOSE NUMÉRICO (TOTAL AL FINAL SIN RESALTADO) ---
                ...
                with rm3:
                    for _, row in summary_metrics.iterrows():
                        col_label = "#4A3B5C" if row['Canal_UI'] == "COSTEÑO" else "#17A2B8"
                        soles_val = row['TOTAL'] if 'TOTAL' in row else row['Dinero']
            ...
            # --- 7. ZONA DE ANÁLISIS PROFUNDO (AUDITORÍA AVANZADA) ---
            ...
            df_devs_reales = df_base_mes[
                ...
                (df_devs_reales['Motivo_Devolucion'].astype(str).str.upper() != "NAN")
            ].copy()
            ...
            if not df_devs_reales.empty:
                ...
                # 🌟 PIPELINE ALGORTÍMICO: Mapear motivos a Macrocategorías Estratégicas
                ...
                # 🌟 NUEVA EXIGENCIA: Gráfico macro ultra-compacto (no ocupe mucho espacio)
                ...
                # Conservación exacta de la lista detallada ya existente por debajo
